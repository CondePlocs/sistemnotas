...
import ModalCalculadoraNotas from '@/components/modals/ModalCalculadoraNotas';

interface ProfesorNavbarProps {
...
export default function ProfesorNavbar({ children }: ProfesorNavbarProps) {
  const { user } = useAuth();
  ...
  const navItems: NavItem[] = [
    ...
    {
      id: 'registros-antiguos',
      label: 'Registros Antiguos',
      href: '/profesor/hojas-anteriores',
      description: 'Ver hojas de trabajo anteriores',
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const isActiveRoute = (href: string) => {
    return pathname === href || (href !== '/profesor/dashboard' && pathname.startsWith(href));
  };
  ...
  return (
    <>
      ...
      <div className="flex h-screen bg-gradient-to-br from-[#FCE0C1] via-[#F6CBA3] to-[#E9E1C9] fixed inset-0">
              ...
              {!isCollapsed && (
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/70">Panel Profesor</p>
                  </div>
                </div>
              )}
              <button
          ...
          <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto scrollbar-hide">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className={`
                  flex-shrink-0 transition-all duration-300 
                  text-[#FCE0C1] group-hover:text-white
                  ${isCollapsed ? 'w-8 h-8 flex items-center justify-center' : ''}
                `}>
                  <div className={`${isCollapsed ? 'scale-125' : ''} transition-transform duration-300`}>
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Contenido Principal */}
        <div className="flex-1 flex flex-col overflow-y-auto bg-gradient-to-br from-[#FCE0C1] via-[#E9E1C9] to-[#D4C5A9]">
          {children}
        </div>
      </div>
